...
function reportForVModel(context, node, firstToken, vModelKey) {
  context.report({
    ...
    fix: (fixer) => {
      return fixer.insertTextAfter(firstToken, ` data-testid="${vModelKey}"`);
    }
  })
}
...
function reportForEvent(context, node, firstToken, reportFixValue) {
  context.report({
    ...
    fix: (fixer) => {
      return fixer.insertTextAfter(firstToken, ` data-testid="${reportFixValue}"`);
    }
  })
}
...
module.exports = {
  meta: {
    ...
    docs: {
      description: "Add data-testid for tag",
  ...
  create: function (context) {
    ...
    return defineTemplateBodyVisitor(context, {
      VElement(node) {
        ...
        if (isAttributePresent(attributes, 'data-testid')) {
          return;
        }
